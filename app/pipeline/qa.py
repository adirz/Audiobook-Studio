...
def word_level_diff(original: str, transcribed: str) -> list[dict]:
    ...
    for op, o_start, o_end, t_start, t_end in matcher.get_opcodes():
        if op == "equal":
            ...
            max_len = max(len(o_slice), len(t_slice))
            for i in range(max_len):
                entry = {"type": "replace", "orig_index": o_start + min(i, len(o_slice) - 1)}
                entry["original"] = o_slice[i] if i < len(o_slice) else ""
                entry["transcribed"] = t_slice[i] if i < len(t_slice) else ""
                diff.append(entry)
        elif op == "insert":
...
def is_pron_substitution(original_word: str, transcribed_word: str,
    ...
    for orig, phonetic in pron_map.items():
        if orig.lower() == lower:
            # The TTS said the phonetic version, which Whisper may transcribe
